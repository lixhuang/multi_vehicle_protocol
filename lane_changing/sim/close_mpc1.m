function env = simple_highway3
    ...
    env.tspan = 0;
    env.sens_r = 30;
    env.lane_center_eps = 0.9;
    env.lane_num = 3;
    env.lane_dic = cell(env.lane_num,1);
    env.lane_center = [-3.7;0;3.7];
    ...
    for k = 1:env.targets_num
        for n = 1:env.lane_num
            if(env.targets(k).q(2) == env.lane_center(n))
                env.lane_dic{n} = [env.lane_dic{n};env.targets(k).q(1), k];
            end
        end
    end
    for n = 1:env.lane_num
        env.lane_dic{n} = sortrows(env.lane_dic{n},'ascend');
        env.lane_dic{n} = env.lane_dic{n}(:,2);
        if(env.q(2) == env.lane_center(n))
            env.q_lane_id = n;
        end
    end
    ...
    env.Sensing = @Sens_range_lane_state; %sensing function
